...
- V1-10.1 已完成：章节多条件检索（前端最小闭环）
  - 章节检索支持关键词（编号/标题/正文）。
  - 新增分卷筛选（全部分卷/指定分卷）。
  - 新增角色筛选（全部角色/指定角色，按角色名与别名匹配章节文本）。
- V1-10.2 已完成：章节状态流（最小闭环）
  - 章节数据新增 `status`（`draft`/`review`/`final`）。
  - 章节编辑页可设置状态。
  - 章节列表支持状态筛选与快速切换状态。
- V1-10.3 已完成：章节版本快照（本地最小版）
  - 写作页新增“快照”侧栏。
  - 可手动保存当前快照（标题/状态/正文/大纲/总结）。
  - 支持按章节维度查看最近快照并一键回填。
- V1-11.1 已完成：导出状态过滤（最小闭环）
  - 导出弹窗支持按状态过滤：全部/草稿/待审/定稿。
  - 后端导出支持状态过滤与范围条件叠加。
  - 导出 Markdown 顶部新增导出条件说明（范围 + 状态）。
